from database import get_db

# Constantes de regras comerciais de desconto
LIMITE_FATURAMENTO_ALTO = 10000.0
TAXA_DESCONTO_ALTO = 0.1
LIMITE_FATURAMENTO_MEDIO = 5000.0
TAXA_DESCONTO_MEDIO = 0.05
LIMITE_FATURAMENTO_BAIXO = 1000.0
TAXA_DESCONTO_BAIXO = 0.02


def criar_pedido(usuario_id, itens):
    db = get_db()
    cursor = db.cursor()

    total = 0

    # Valida estoque e calcula total usando query parametrizada
    for item in itens:
        cursor.execute("SELECT * FROM produtos WHERE id = ?", (item["produto_id"],))
        produto = cursor.fetchone()
        if produto is None:
            return {"erro": f"Produto {item['produto_id']} não encontrado"}
        if produto["estoque"] < item["quantidade"]:
            return {"erro": f"Estoque insuficiente para {produto['nome']}"}
        total += (produto["preco"] * item["quantidade"])

    # Cria o pedido
    cursor.execute(
        "INSERT INTO pedidos (usuario_id, status, total) VALUES (?, 'pendente', ?)",
        (usuario_id, total)
    )
    pedido_id = cursor.lastrowid

    # Insere itens do pedido e atualiza estoque com queries parametrizadas
    for item in itens:
        cursor.execute("SELECT preco FROM produtos WHERE id = ?", (item["produto_id"],))
        produto = cursor.fetchone()
        
        cursor.execute(
            "INSERT INTO itens_pedido (pedido_id, produto_id, quantidade, preco_unitario) VALUES (?, ?, ?, ?)",
            (pedido_id, item["produto_id"], item["quantidade"], produto["preco"])
        )

        cursor.execute(
            "UPDATE produtos SET estoque = estoque - ? WHERE id = ?",
            (item["quantidade"], item["produto_id"])
        )

    db.commit()
    return {"pedido_id": pedido_id, "total": total}

def get_pedidos_usuario(usuario_id):
    db = get_db()
    cursor = db.cursor()
    
    # Query parametrizada para buscar pedidos
    cursor.execute("SELECT * FROM pedidos WHERE usuario_id = ?", (usuario_id,))
    pedidos_rows = cursor.fetchall()
    
    if not pedidos_rows:
        return []

    # Mapeia pedidos
    pedidos_list = []
    pedidos_map = {}
    for row in pedidos_rows:
        pedido = {
            "id": row["id"],
            "usuario_id": row["usuario_id"],
            "status": row["status"],
            "total": row["total"],
            "criado_em": row["criado_em"],
            "itens": []
        }
        pedidos_list.append(pedido)
        pedidos_map[row["id"]] = pedido

    # Resolve N+1 buscando todos os itens e nomes de produtos de uma só vez usando JOIN e IN clause
    pedido_ids = list(pedidos_map.keys())
    placeholders = ",".join("?" for _ in pedido_ids)
    
    query = f"""
        SELECT ip.pedido_id, ip.produto_id, ip.quantidade, ip.preco_unitario, p.nome AS produto_nome
        FROM itens_pedido ip
        LEFT JOIN produtos p ON ip.produto_id = p.id
        WHERE ip.pedido_id IN ({placeholders})
    """
    
    cursor.execute(query, tuple(pedido_ids))
    itens_rows = cursor.fetchall()
    
    for item in itens_rows:
        ped_id = item["pedido_id"]
        if ped_id in pedidos_map:
            pedidos_map[ped_id]["itens"].append({
                "produto_id": item["produto_id"],
                "produto_nome": item["produto_nome"] if item["produto_nome"] else "Desconhecido",
                "quantidade": item["quantidade"],
                "preco_unitario": item["preco_unitario"]
            })
            
    return pedidos_list

def get_todos_pedidos():
    db = get_db()
    cursor = db.cursor()
    
    cursor.execute("SELECT * FROM pedidos")
    pedidos_rows = cursor.fetchall()
    
    if not pedidos_rows:
        return []

    pedidos_list = []
    pedidos_map = {}
    for row in pedidos_rows:
        pedido = {
            "id": row["id"],
            "usuario_id": row["usuario_id"],
            "status": row["status"],
            "total": row["total"],
            "criado_em": row["criado_em"],
            "itens": []
        }
        pedidos_list.append(pedido)
        pedidos_map[row["id"]] = pedido

    pedido_ids = list(pedidos_map.keys())
    placeholders = ",".join("?" for _ in pedido_ids)
    
    query = f"""
        SELECT ip.pedido_id, ip.produto_id, ip.quantidade, ip.preco_unitario, p.nome AS produto_nome
        FROM itens_pedido ip
        LEFT JOIN produtos p ON ip.produto_id = p.id
        WHERE ip.pedido_id IN ({placeholders})
    """
    
    cursor.execute(query, tuple(pedido_ids))
    itens_rows = cursor.fetchall()
    
    for item in itens_rows:
        ped_id = item["pedido_id"]
        if ped_id in pedidos_map:
            pedidos_map[ped_id]["itens"].append({
                "produto_id": item["produto_id"],
                "produto_nome": item["produto_nome"] if item["produto_nome"] else "Desconhecido",
                "quantidade": item["quantidade"],
                "preco_unitario": item["preco_unitario"]
            })
            
    return pedidos_list

def atualizar_status_pedido(pedido_id, novo_status):
    db = get_db()
    cursor = db.cursor()
    cursor.execute(
        "UPDATE pedidos SET status = ? WHERE id = ?",
        (novo_status, pedido_id)
    )
    db.commit()
    return True

def relatorio_vendas():
    db = get_db()
    cursor = db.cursor()

    cursor.execute("SELECT COUNT(*) FROM pedidos")
    total_pedidos = cursor.fetchone()[0]

    cursor.execute("SELECT SUM(total) FROM pedidos")
    faturamento = cursor.fetchone()[0]
    if faturamento is None:
        faturamento = 0

    cursor.execute("SELECT COUNT(*) FROM pedidos WHERE status = 'pendente'")
    pendentes = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM pedidos WHERE status = 'aprovado'")
    aprovados = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM pedidos WHERE status = 'cancelado'")
    cancelados = cursor.fetchone()[0]

    # Regras de desconto comerciais parametrizadas
    desconto = 0
    if faturamento > LIMITE_FATURAMENTO_ALTO:
        desconto = faturamento * TAXA_DESCONTO_ALTO
    elif faturamento > LIMITE_FATURAMENTO_MEDIO:
        desconto = faturamento * TAXA_DESCONTO_MEDIO
    elif faturamento > LIMITE_FATURAMENTO_BAIXO:
        desconto = faturamento * TAXA_DESCONTO_BAIXO

    return {
        "total_pedidos": total_pedidos,
        "faturamento_bruto": round(faturamento, 2),
        "desconto_aplicavel": round(desconto, 2),
        "faturamento_liquido": round(faturamento - desconto, 2),
        "pedidos_pendentes": pendentes,
        "pedidos_aprovados": aprovados,
        "pedidos_cancelados": cancelados,
        "ticket_medio": round(faturamento / total_pedidos, 2) if total_pedidos > 0 else 0
    }
